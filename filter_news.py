...
def load_api_key() -> str:
    ...
    if not raw or raw.startswith("sk-ant-여기에"):
        print("[오류] ANTHROPIC_API_KEY 가 설정되지 않았습니다.")
        print(f"       1) {BASE_DIR / '.env.example'} 를 .env 로 복사")
        print(f"       2) .env 안의 키 값을 실제 API 키로 교체")
        sys.exit(1)

    # 키 자동 정제 — 앞뒤·줄바꿈에 잡내용(다음 줄, 변수명 등)이 붙어도 순수 키만 추출
    # 표준 Anthropic 키 패턴: sk-ant-...  (영문/숫자/하이픈/언더스코어)
    m = re.search(r"sk-ant-[A-Za-z0-9_\-]+", raw)
    key = m.group(0) if m else raw

    if len(key) != len(raw):
        print(f"[정제] 입력값에 잡내용 발견 → 원본 {len(raw)}자 에서 순수 키 {len(key)}자 추출")
    print(f"[진단] API 키 OK — 길이={len(key)}, 시작={key[:12]!r}, 끝={key[-4:]!r}")
    return key
...
def score_articles(client: Anthropic, articles: list[dict], profile: str) -> list[dict]:
    ...
    system_prompt = (
        "당신은 '양보다 질'을 추구하는 정보 큐레이터입니다. "
        "단순히 주제가 관심사와 맞는지가 아니라, **그 기사가 사용자에게 실질적 통찰을 주는지**를 1~10 점수로 평가합니다.\n\n"

        "## 핵심 원칙\n"
        "주제가 관심사(AI·마케팅·자동화 등)와 맞아도 **홍보·광고·과장성 기사는 낮은 점수**를 줍니다.\n"
        "반대로 키워드가 직접 없어도 **구조·원리·거시 흐름을 주는 기사는 높은 점수**를 줍니다.\n\n"

        "## 가점 신호 (이런 성격이면 높은 점수)\n"
        "- 작동 원리·구조·'왜 그런지'를 설명하는 기사\n"
        "- 검증 가능한 데이터·실측·통계 기반\n"
        "- 독자가 직접 적용·실행할 수 있는 구체적 방법론\n"
        "- 개인이 막을 수 없는 거시적 시장·정책·기술 변화 (알아둬야 할 흐름)\n"
        "- 실패·한계·리스크도 균형 있게 다루는 기사\n\n"

        "## 감점 신호 (이런 성격이면 낮은 점수, 주제가 맞아도 3점 이하)\n"
        "- 특정 상품·서비스·종목·코인 홍보가 주목적인 기사 (기사형 광고)\n"
        "- '쉽게 / 월 ○○만원 / 누구나 / 지금 안 하면 손해' 식 과장·조급증 유발\n"
        "- 출처·근거 없이 막연한 트렌드만 나열\n"
        "- 협찬·광고성 콘텐츠, 특정 업체 띄우기\n\n"

        "## 점수 가이드\n"
        "- 10점: 관심 영역의 핵심 + 구조적 통찰·중요 흐름 (홍보성 전혀 없음)\n"
        "- 8~9점: 관심 영역에 직접 관련 + 실질 정보 가치 있음\n"
        "- 6~7점: 간접 관련이거나, 관련은 높지만 정보 깊이가 얕음\n"
        "- 4~5점: 약하게 연관 또는 홍보성이 일부 섞임\n"
        "- 1~3점: 무관하거나, 주제는 맞아도 명백한 광고·과장·종목추천\n\n"
